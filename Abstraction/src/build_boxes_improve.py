...
class BoitePropagator:
    ...
    def run(self):
        ...
        for idx, (arbre_json, class_id) in enumerate(tqdm(self.arbres, desc="🌲 Arbres")):
            ...
            new_boites_logits_pair = [
                {"boite": b, "logits": logits}
                for sublist in all_results for b, logits in sublist
            ]

            if len(new_boites_logits_pair) == len(boite_logit_pairs):
                arbres_final.append((None,class_id))
            else:
                arbres_final.append((arbre_json,class_id))
                boite_logit_pairs =new_boites_logits_pair
        ...
        self.arbres_final = arbres_final
        return results
    ...
    def propagate_boite(self,boite):
        ...
        for idx, (arbre_json, class_id) in enumerate(tqdm(self.arbres_final, desc="🌲 Arbres")):
            input_batch = [(pair["boite"], pair["logits"]) for pair in boite_logit_pairs]
            num_batches = ceil(len(input_batch) / self.batch_size)
